...
检测到 ⚠️ 状态的工具时，**不只是报告，而是主动帮用户完成认证**。

**处理逻辑**（按优先级依次处理）：

**xreach（Twitter）⚠️ 已安装但未认证时**：
```
检测到 xreach 未认证。Twitter 是最有价值的一手数据源。
要我帮你运行认证吗？只需在弹出的浏览器中登录 Twitter 账号。
```
→ 用户同意后：执行 `xreach auth`，等待用户在浏览器中完成授权
→ 认证成功后：更新状态为 ✅，继续下一个
→ 用户拒绝：标注 🔴 降级，继续

**mcporter（小红书）⚠️ CLI 已装但 MCP 服务未运行时**：
```
检测到 mcporter 已安装但 MCP 服务未启动。
要我帮你在后台启动吗？
```
→ 用户同意后：执行 `mcporter &` 或使用后台任务启动 mcporter
→ 等待几秒后检测 `curl -sf http://localhost:18060/` 确认启动成功
→ 首次启动可能需要扫码：提示用户查看终端输出完成扫码
→ 用户拒绝：标注 🔴 降级，继续

**gh（GitHub）⚠️ 已安装但未认证时**：
```
检测到 gh 未认证。要我帮你运行认证吗？
```
→ 用户同意后：执行 `gh auth login`，引导用户在浏览器中完成
→ 用户拒绝：标注 🔴 降级，继续

**yt-dlp（YouTube/B站）✅ 已安装**：
→ 不弹认证提示（无命令可执行，只需 Chrome 已登录）
→ 状态面板附注"请确保 Chrome 已登录 YouTube/B站 账号"

**重要**：认证协助是**逐个进行**的，不一次性弹出所有提示。每完成一个认证，立即更新状态。如果用户说"跳过"或"之后再说"，剩余全部跳过，直接进入采集。

#### 阶段 4：输出状态面板

所有认证处理完毕后，输出最终状态面板（这是**采集前的最后一个输出**）：

```
📡 内容雷达启动

=== 工具状态 ===
✅ xreach — Twitter/X（刚刚认证成功）
✅ yt-dlp — YouTube/B站（请确保 Chrome 已登录）
✅ feedparser — RSS 订阅（自动安装成功）
✅ mcporter — 小红书/Exa（刚刚启动成功）
✅ miku-ai — 微信公众号
⚠️ gh — 已跳过认证，GitHub 数据用网页搜索替代
❌ agent-reach — 未安装，抖音数据用网页搜索替代
...
- 状态面板在所有认证完成后、采集开始前输出
- 能自动安装的**默认安装**，不问用户
- 需要认证的**主动提供协助**，用户同意才执行
- 用户说"跳过"→ 剩余认证全部跳过，不再逐个询问
- 安装失败时显示错误原因 + 手动安装命令
