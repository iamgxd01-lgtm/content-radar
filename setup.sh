...
echo ""
echo -e "${BOLD}🔑 认证设置（可选，让数据更完整）${NC}"
echo ""
echo "  不认证也能用！未认证的渠道会自动降级到网页搜索。"
echo "  但认证后能拿到一手数据，选题质量更高。"
echo ""
echo -e "  ${BOLD}Twitter/X${NC}（获取实时讨论，最推荐认证）"
echo "    运行：xreach auth"
echo "    效果：浏览器自动打开，登录 Twitter 账号即可"
echo ""
echo -e "  ${BOLD}YouTube / B站${NC}（获取视频数据）"
echo "    只需：在 Chrome 浏览器中登录你的 YouTube / B站 账号"
echo "    无需运行任何命令，yt-dlp 会自动读取 Chrome 的登录状态"
echo ""
echo -e "  ${BOLD}小红书${NC}（获取热帖和评论）"
echo "    运行：mcporter"
...
echo -e "${BOLD}🚀 下一步${NC}"
echo ""
echo "  1. 完成上面的认证（可选，也可以之后再做）"
echo "  2. 打开你的 AI 编辑器（Claude Code / Qwen Code / Codex 等）"
echo "  3. 输入 \"内容雷达\" → 全网扫描找选题"
echo "  4. 输入 \"拆解 [URL]\" → 深度拆解一条爆款内容"
echo "  5. 首次运行会问你 3 个问题来了解你的定位，之后全自动"
echo ""
echo -e "${BOLD}📢 本版亮点${NC}"
echo ""
